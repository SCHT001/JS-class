...
function getResult() {
  ...
  const tokens = state.expression.match(/(\d+(\.\d+)?|\+|\-|\*|\/|\%)/g);
  // const tokens = state.expression.match(/(\-\d+)|(\d+(\.\d+)?|\+|\-|\*|\/|\%)/g);
  console.log(tokens);
  const output = [];
  const operatorStack = [];   
...
// checking if is %
        if(token=="%"){
        result=state.operators[token](b);
        }
        else{
          result = state.operators[token](a, b);
        }
  ...
  btn.addEventListener("click", function (e) {
    ...
    switch (clickedBtn) {
      ...
      default:
        ...
        if(!operatorClicked && isOperator(clickedBtn)){
